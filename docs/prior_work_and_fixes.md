...
| F2 | absence-of-evidence negatives | Treat as **positive-unlabeled (PU)** learning, not clean 0/1; or at minimum report sensitivity to negative definition. Restrict negatives to **plausible hosts** (vertebrate/mammal-associated) so we're not just sorting plant viruses from animal viruses. | 🟡 host-class flags in `build_labels.py`; PU ⬜ |
